...
dataentery_frame_bg = '#E0FFFF'
buttons_frame_bg = '#B0C4DE'
hlb_btn_bg = '#4682B4'
...
Button(data_entry_frame, text='Add expense', command=lambda: add_another_expense(), font=btn_font, width=30, bg=hlb_btn_bg).place(x=10, y=395)
Button(data_entry_frame, text='Convert to words before adding', command=lambda: expense_to_words_before_adding(), font=btn_font, width=30, bg=hlb_btn_bg).place(x=10,y=450)

# Buttons' Frame
Button(buttons_frame, text='Delete Expense', font=btn_font, width=25, bg=hlb_btn_bg, command=lambda: remove_expense()).place(x=30, y=5)
Button(buttons_frame, text='Clear Fields in DataEntry Frame', font=btn_font, width=25, bg=hlb_btn_bg, command=lambda: clear_fields()).place(x=335, y=5)
Button(buttons_frame, text='Delete All Expenses', font=btn_font, width=25, bg=hlb_btn_bg, command=lambda: remove_all_expenses()).place(x=640, y=5)
Button(buttons_frame, text='View Selected Expense\'s Details', font=btn_font, width=25, bg=hlb_btn_bg, command=lambda: view_expense_details()).place(x=30, y=65)
Button(buttons_frame, text='Edit Selected Expense', command=lambda: edit_expense(), font=btn_font, width=25, bg=hlb_btn_bg).place(x=335,y=65)
Button(buttons_frame, text='Convert Expense to a sentence', font=btn_font, width=25, bg=hlb_btn_bg, command=lambda: selected_expense_to_words()).place(x=640, y=65)
...
def add_another_expense():
    ...
    connector.execute('INSERT INTO ExpenseTracker (Date, Payee, Description, Amount, ModeOfPayment) VALUES (?, ?, ?, ?, ?)',
...
def remove_expense():
    ...
    if surety:
        ...
        connector.commit()
        list_all_expenses()
        mb.showinfo('Record deleted successfully!', f'Record of {values_selected[2]} deleted successfully!')

def remove_all_expenses():
    surety = mb.askyesno('Are you sure?', 'Are you sure that you want to delete all records from the Expense Tracker?')
    
    if surety:
        connector.execute('DELETE FROM ExpenseTracker')
        connector.commit()
        list_all_expenses()
        mb.showinfo('All records deleted successfully!', 'All expense records have been deleted successfully!')
...
def edit_expense():
    ...
    expenditure_date = datetime.date(int(values_selected[1][:4]), int(values_selected[1][5:7]), int(values_selected[1][8:]))
    date.set_date(expenditure_date)
    payee.set(values_selected[2])
    desc.set(values_selected[3])
    amnt.set(values_selected[4])
    MoP.set(values_selected[5])

    connector.execute('UPDATE ExpenseTracker SET Date=?, Payee=?, Description=?, Amount=?, ModeOfPayment=? WHERE ID=?',
                     (date.get_date(), payee.get(), desc.get(), amnt.get(), MoP.get(), values_selected[0]))
    connector.commit()
...
def expense_to_words_before_adding():
    # This function will convert expense to words before adding
    # Implementation can be added here based on your requirements
    pass

def selected_expense_to_words():
    # This function will convert the selected expense to words
    # Implementation can be added here based on your requirements
    pass

# Finalizing the GUI window
root.update()
list_all_expenses()  # Initial load of expenses
